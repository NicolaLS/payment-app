...
**Document status:** Static architecture audit and rewrite decision record, updated for the single-connection product and planned app split

**Last updated:** 2026-07-21

**Repository scope:** Kotlin Multiplatform app in `app/`

**Product assumption:** The current app allows one connection at a time, either Blink or NWC. The rewrite will ship as two separately packaged apps: one Blink-only and one NWC-only.
...
The app has already solved several difficult product problems well: fixed and dynamic payment requests receive different duplicate handling; uncertain payment outcomes are recognized as distinct from definitive failure; the NWC connection is reused and tied to lifecycle; and the real E2E environment exercises an actual payment path. Those invariants should become first-class specifications for the rewrite.

The current mechanisms around those ideas are much weaker. Weak internal ownership boundaries, a 1,786-line payment ViewModel, loosely implemented MVI, session-only attempt tracking, secret-bearing navigation arguments, incomplete protocol validation, JSON blobs used as a database, inconsistent error channels, custom lifecycle/scoping infrastructure, and stale CI create avoidable correctness and security risks.

The 2026-07-21 single-connection refactor removed wallet collections, active-wallet selection, `WalletPaymentTarget`, per-wallet Blink credential keys, and the multi-client NWC cache. That is a useful product simplification, but it also invalidates parts of the original audit and removed the explicit connection snapshot that previously protected pending work. The planned Blink/NWC app split goes further: runtime provider routing should not be rebuilt in the greenfield architecture. Shared code should express provider-neutral payment behavior, while each packaged app binds exactly one provider implementation at compile time.
...
- Payment parsing, confirmation, execution, duplicate detection, pending resolution, and connection replacement/removal.
...
| Single optional connection | 4 | Correctly removes unsupported multi-wallet complexity; replacement/removal still needs an immutable connection generation per attempt |
| Transitional mixed-provider routing | 2 | `PaymentService` still selects Blink or NWC from mutable connection state; do not reproduce this router after the app split |
| Separate Blink and NWC application boundaries | 3 | Correct target direction, but build, storage, migration, deep-link, release, and test boundaries are not designed yet |
...
| NWC connection reuse and lifecycle | 4 | One of the strongest infrastructure decisions; bind the cached client to a connection generation and make cancellation policy explicit |
...
| Settings JSON blobs for connection/contacts/shortcuts | 2 | Acceptable prototype storage, not a transactional or safely migratable application database |
...
| Current DI graph and service-locator use | 2 | A 430-line module, hidden lookups, circular callbacks, and unqualified scopes/dispatchers obscure ownership |
...
### 4.1 A payment attempt owns its connection generation

Before the single-connection refactor, the app snapshotted a concrete `WalletPaymentTarget` before submitting a payment. That type and its explicit target parameters were removed. The current product prevents a second simultaneous connection, but removal and later reconnection are still possible. Submission and reconciliation now read mutable global connection state, so pending work can lose the credential context that created it.

The two-app split makes provider choice a compile-time application property; it does not make connection identity unnecessary. Give every successful provisioning a new opaque `ConnectionId` (or monotonically unique generation), persist it on the attempt before any external request, and pass it to submission and lookup. Removing or replacing a connection must mark its non-final attempts for reconciliation with the original credential when possible, or leave them honestly `Unknown`; it must never look them up through a new connection.
...
`NwcConnectionManager` caches one client, serializes creation with a mutex, proactively connects, and evicts it on lifecycle/removal events. Reusing the relay connection is both a latency and resource-management win. Preserve the concept, associate the client with an opaque `ConnectionId`/generation so a stale client cannot be reused after replacement, and specify what happens to in-flight work when the app backgrounds.
...
### 4.9 Provider choice belongs to the packaged app

The planned products are not one app with a provider toggle. The Blink app should not link NWC code or expose NWC onboarding/deep links, and the NWC app should not link Blink/Apollo code or expose Blink credential flows. Share payment policy, protocol parsing, state machines, UI, and test contracts; bind one backend, one onboarding branch, one privacy inventory, and one release identity in each app composition. A build-time leakage test should fail if either provider implementation enters the other app artifact.

## 5. Current architecture

The root project contains `:shared` and `:androidApp`; iOS is an Xcode shell. Nearly all domain, both provider implementations, presentation, navigation, resources, and composition code lives in `:shared`. The app currently stores one optional `WalletConnection`, whose `WalletType` selects Blink or NWC at runtime.
...
shared
  ...
  domain models + use cases + repository interfaces
        ^
        |
  Blink + NWC data implementations and provider clients
        ^
        |
...
- Common production Kotlin is approximately 21,907 lines; common tests are approximately 5,174 lines.
- `MainViewModel.kt` is 1,786 lines.
- `SettingsNavigation.kt` is 1,077 lines; `PayNavigation.kt` is 723 lines.
- `PaymentsSettingsScreen.kt` is 940 lines and its ViewModel is 644 lines.
- `BlinkApiClient.kt` is 561 lines; `PendingPaymentTracker.kt` is 465 lines.
- `NwcModule.kt` is 430 lines and composes nearly the whole app despite its provider-specific name.
- Native scanner implementations are approximately 586 Android lines and 1,198 iOS lines.
- There are approximately 45 use-case classes across 33 files, totaling about 481 lines, indicating that many are thin delegation wrappers.
...
For the current app, keep the conventional `:shared` module while safety fixes land. For the rewrite, the two independently shipped provider apps are a demonstrated reason for a small number of enforceable module boundaries:

```text
:core                    pure values, protocol rules, workflows, state machines, ports
:shared-ui               feature-owned Compose UI, navigation contracts, resources
:provider-nwc            NWC provisioning, session, payment adapter
:provider-blink          Blink provisioning, Apollo operations, payment adapter

:androidNwcApp / NWC iOS shell       core + shared-ui + provider-nwc
:androidBlinkApp / Blink iOS shell   core + shared-ui + provider-blink
```

The exact Gradle/Xcode names are not important; compile-time provider exclusion is. Core must not import UI or infrastructure, provider modules must not depend on one another, and app compositions must bind exactly one `PaymentBackend`. Keep feature packages inside `:core`/`:shared-ui` until ownership or build evidence justifies another split. Do not recreate dozens of modules.
...
- Recoverability/control: reconnect, retry, or request a new invoice.
...
Connection IDs, wallet public keys, payment hashes, NWC URIs, Blink API keys, contact IDs, and shortcut IDs are mostly strings. This permits accidental interchange and accidental `toString()` disclosure. `Bolt11Memo.HashOnly(ByteArray)` also inherits array identity equality rather than content equality.
...
### 7.5 Connection model — rating 2/5

The one-wallet refactor correctly deleted `WalletPaymentTarget` and the wallet collection, but `WalletConnection` remains a tagged union with a `WalletType`, an NWC bearer URI, a field named `walletPublicKey`, provider metadata, and defaults that permit invalid combinations. For Blink, “public key” is a generated constant-like local identifier rather than a wallet public key. Provider selection and credential-bearing state therefore still leak into the common model.

Each future app needs one optional provider-specific connection, not a common multi-provider wallet aggregate. Keep only shared lifecycle identity/status in core and let each provider module own its configuration:
...
// :provider-nwc
data class NwcConfiguration(val relays: List<RelayUrl>, val walletPubkey: PublicKey)

// :provider-blink
data class BlinkConfiguration(val accountId: BlinkAccountId)
```

The app-specific credential vault stores secret material by `ConnectionId`; navigation, UI state, logs, saved state, and durable public records only carry the ID. A new ID is issued on every disconnect/reconnect so pending attempts cannot bind to replacement credentials. `ConnectionStatus.NeedsReauthentication` is safer than automatically deleting a profile on a heuristically classified authentication failure.
...
```text
AttemptId
connectionId + app/provider provenance
...
### 8.3 Payment routing and providers — rating 2/5 current, unnecessary in the split apps

`PaymentService` still links both provider repositories, seeds mutable current-connection state with `runBlocking` in its constructor, and branches on `WalletType` for every operation. Constructor blocking is unsafe and hides I/O in object creation. More importantly, removing `WalletPaymentTarget` means submission and lookup can consult a connection different from the one that created a pending attempt. The product now blocks a second simultaneous connection, but disconnect/reconnect remains enough to expose this identity problem.

Do not replace this with another registry. Define one shared port and bind exactly one implementation in each app:

```kotlin
interface PaymentBackend {
    suspend fun submit(connection: ConnectionId, invoice: ValidatedInvoice): SubmitPaymentOutcome
    suspend fun lookup(connection: ConnectionId, hash: PaymentHash): LookupPaymentOutcome
}
```

The NWC app injects `NwcPaymentBackend`; the Blink app injects `BlinkPaymentBackend`. The application coordinator resolves the sole connection once, records its ID on the attempt, and passes that ID on every call. Provider implementations resolve credentials at the last responsible moment through their app-specific vault. There is no `WalletProvider`, provider registry, active selection, or runtime provider branch in shared payment code.
...
This is a good example of an implementation whose central decision should survive. Single-client reuse, a mutex around creation, foreground prewarming, explicit pay/lookup timeouts, failure mapping, and background/removal cleanup all address real Nostr relay behavior.

Refinements needed:

- The cache is now a single nullable client with no recorded connection generation. Associate it with `ConnectionId` and verify that ID before reuse; resolve the bearer URI only inside the session factory.
...
- Prewarming exposes presence to a relay and consumes battery/network. Keep it only as a documented latency/privacy trade-off for the connected NWC account.
- Capability/encryption metadata lacks a fetched timestamp or explicit refresh policy. If refresh is added, its write must be conditional on the same `ConnectionId` still being current.
...
- Credentials are attached per request rather than being baked into generated GraphQL operations.
...
- Existing tests cover a useful set of authorization, status, and error cases.
...
- Provisioning writes the global API-key/default-wallet entries before the connection profile. A later storage failure leaves orphaned credentials.
...
- The cached global default-wallet identity lacks freshness/version semantics.
...
3. Create a new `ConnectionId` and public profile transaction.
4. Store the credential under that ID.
5. Make the connection available only after both stores succeed.
...
The single connection, contacts, and shortcuts are currently JSON documents in `Settings`. This has several failure modes:

- Contact/shortcut read-transform-write operations have no transaction; concurrent updates can overwrite one another. The single connection now has a mutex, but its persistence is not transactional with credential work.
- The connection profile and its provider credential are separate writes, allowing torn state after crash.
- Corrupt JSON silently becomes a missing connection or empty contact aggregate and can orphan credentials.
- There is no explicit schema version or migration ledger.
- The one-wallet migration reads `wallet.list`, retains the former active wallet (or first entry), writes `wallet.connection`, deletes the old list/selection keys, and deletes discarded Blink credential keys during initialization. It has no durable checkpoint, recovery UI, or rollback window; discarded NWC URIs disappear with the old aggregate.
- Unknown future connection types can be reinterpreted unsafely during downgrade rather than quarantined.
...
Use a KMP relational database such as SQLDelight for the single public connection profile, contacts, shortcuts, attempts, and migration ledger. Use database transactions, foreign keys, migration tests, per-record validation/quarantine, and injected `Clock`/`IdGenerator`. Preferences remain a small typed store, preferably versioned per feature. Do not add a wallet collection or active-selection table unless multi-wallet becomes a real requirement again.
...
Expose a narrow provider-specific `CredentialVault` that only stores the NWC URI or Blink API key/default account by `ConnectionId`. Store public application data elsewhere. Bind ciphertext to the credential key/record identity, handle invalidated keys explicitly, define iOS accessibility class, and document backup, uninstall, migration, reset, and rotation behavior. Each packaged app must use its own storage namespace unless an explicitly designed, platform-entitled transfer mechanism is required. Prefer an audited platform/library wrapper over expanding custom general-purpose cryptography.
...
The current 430-line `nwcModule` wires far more than NWC. It registers storage, network, both providers, the runtime router, domain services, use cases, presentation controllers, and ViewModels. Direct calls to `KoinPlatformTools.defaultContext().get()` throughout app/navigation/composable code turn the container into a service locator. A connection-removal callback resolves `NwcConnectionManager` through Koin, forming a hidden circular dependency with connection settings.

Use Koin only at composition boundaries:

- Separate common Koin definition groups such as `platformModule`, `storageModule`, `networkModule`, `paymentModule`, and feature modules, plus exactly one provider module selected by the app composition.
...
- Graph-verification tests that create every production component for both Blink and NWC Android/iOS app compositions where feasible and assert that the other provider is absent.
- No container access from domain/application classes and no callback that resolves a dependency later.

Replace the circular removal callback with a `ConnectionLifecycle` application workflow that transactionally coordinates profile, credential, provider session, and attempt invariants. There is no active-selection concern in either future app.
...
The journey has sound product elements: a nested graph, explicit agreement, provider education, and completion tied to a usable connection. Current coordination is fragile:
...
- Wallet type selection renders selection state but navigation immediately advances and passes no selected value, leaving the radio state effectively dead. Remove this branch in the split apps; each app should explain and provision only its own provider.
...
- Connection, contact, and shortcut deletion is immediate with no confirmation or undo, even though connection deletion also affects credentials, live sessions, and pending reconciliation.
...
Split preference, contact editor, shortcut editor, import, and connection-management owners. Choose either explicit Save/Cancel or a genuinely specified autosave contract with debounce, version/conflict handling, and visible save/error state. Use bulk transactional import and report partial duplicates/validation before commit. Destructive connection removal should explain consequences and offer confirmation; local contact/shortcut removal should at least support undo where feasible. Clipboard access must be an explicit Paste action.
...
The separate Android E2E application ID and release-like/minified build are strong. The former Android/iOS wallet seeders were removed in favor of real onboarding, which reduces storage-schema duplication and is a good correction. Keep only narrow test hooks such as payment-input injection in E2E-only targets. The split requires a separately installable E2E artifact and smoke suite for each provider app; the NWC app additionally keeps the real regtest payment path.
...
- Blink API/repository tests for authorization, statuses, and errors.
...
3. **Port contract suites:** the same payment behavior contract executed independently against NWC and Blink implementations, including cancellation and unknown outcomes.
...
7. **E2E/regtest:** each app's onboarding and provider-specific deep links; fixed/amountless/LNURL; threshold boundary; disconnect/reconnect during an attempt; duplicate scan; provider timeout then reconciliation; process restart; revoked credential; and failure recovery.
8. **Architecture gates:** forbidden imports/module dependencies, provider-artifact leakage, both app DI graphs, Apollo schema drift, dependency verification, secret scan, and per-app release configuration checks.
...
- Structured events with a random opaque `AttemptId`, state transition, app/provider build identity, duration bucket, and stable error code.
...
Create a real data-flow inventory before the rewrite ships. For each datum, record source, purpose, recipient, transport, device/server retention, backup behavior, user deletion, sensitivity, and whether it appears in diagnostics. Produce a policy/inventory for each packaged app: the Blink app should not claim Nostr relay processing, and the NWC app should not claim Blink API processing. Update the public language to distinguish “Lasr has no first-party analytics/backend” from “the app sends data to services necessary to perform user-requested features.” Avoid absolute security claims.
...
```text
                         shared source
        +------------------------------------------------+
        | core/payment + shared UI + common platform ports|
        +----------------------+-------------------------+
                               |
                  shared PaymentBackend port
                    /                         \
       :provider-nwc                           :provider-blink
     NWC session + vault                    Apollo + Blink vault
             |                                      |
   NWC Android/iOS composition             Blink Android/iOS composition
   NWC IDs, links, policy, CI               Blink IDs, links, policy, CI
```

These are separate products built from shared source, not flavors that can select a provider at runtime. Each composition has its own application/bundle ID, display name, icons, deep-link ownership, privacy declaration, storage namespace, release signing, store record, diagnostics label, and E2E artifact. One may use Gradle product flavors internally if that keeps the build maintainable, but release tasks must produce two explicit, independently verifiable applications and compile only the selected provider module.
...
- `application` owns workflows, transaction boundaries, state machines, policies, and provider-neutral ports. It does not format UI text or choose a provider.
...
- Shared presentation/navigation owns reusable routes, theme, and feature UI. App compositions own their provider-specific onboarding graph, supported app links, and root dependency graph.
- Platform shells own native initialization, entitlements/permissions, app-specific secure facilities, lifecycle adapters, and release identity.
- Provider code cannot be referenced by the other provider module or packaged app. Shared code cannot branch on `WalletType`, build flavor, or bundle/application ID.
...
Input source
  ...
  -> create durable PaymentAttempt(connectionId, fingerprint)
  -> submit through the app's single injected provider backend
  -> persist outcome
...
The database record is created before submission. The credential vault is accessed only inside the provider adapter and only for the attempt's `ConnectionId`. Navigation observes IDs and renders; it never transports payment secrets or owns settlement truth.
...
- SQL database: one public connection profile/capabilities record, contacts, shortcuts, attempts, and migration ledger.
...
- Credential vault: the app's one credential kind only, addressed by `ConnectionId` and versioned.
...
### 17.5 Provider and app-composition model

Define one backend contract in shared application code and implement it once per provider module. Each packaged app binds its implementation directly; there is no sealed provider dispatch or backend registry. Provider-only capabilities remain behind provider-owned feature ports and screens rather than polluting the core contract with nullable methods. Run the same payment contract suite against both implementations, then run artifact tests proving that Blink builds contain no NWC SDK/permissions/routes and NWC builds contain no Apollo operations/Blink endpoints/routes.

### 17.6 Split identity and installed-user migration

Only one successor can transparently inherit the current Android application ID, iOS bundle ID, signing lineage, private sandbox, Keychain service behavior, universal/app links, and store listing. Before implementation, decide which app is the in-place successor. The other app is a new install and cannot assume access to the original app's database or secrets.

Use this migration rule:

1. Inspect the single retained legacy connection without deleting it.
2. If its provider matches the in-place successor, import it transactionally into that app's new schema.
3. If it belongs to the other provider, present an explicit handoff/reconnect path. Never copy an NWC bearer URI or Blink API key through a normal deep link, pasteboard, log, or analytics event.
4. Keep a rollback-safe migration marker and the legacy record until the imported profile and credential can be read and the product-approved rollback window closes.
5. Namespace databases, preferences, Keychain services/access groups, Android backup rules, app links, and E2E fixtures per app. Share storage only through an explicitly threat-modeled platform mechanism.

The current one-wallet migration has already collapsed `wallet.list` into `wallet.connection` and discarded non-retained entries. Golden fixtures must cover both the old multi-wallet schema and the new single-connection schema because installed users can arrive from either version.
...
| 002 | Shared-module package boundaries | Small shared core/UI plus provider-specific modules; enforce compile-time exclusion between packaged apps |
...
| 005 | Connection and credential model | One optional opaque `ConnectionId` per app; public profile separate from narrowly vaulted secret; new ID on reconnect |
...
| 015 | Quality gates | Conformance, provider contracts, both app graphs/artifacts, migrations, architecture, accessibility, and regtest CI |
| 016 | Backup/reinstall/recovery | Explicit Android backup exclusions and iOS Keychain retention/accessibility behavior |
| 017 | External identity/release provenance | Choose the in-place successor; give both apps explicit store/signing/link identities and isolated reproducible builds |
| 018 | Split migration and data ownership | Support old multi-wallet and current single-connection inputs; no implicit cross-app secret sharing |
...
5. Restore an immutable connection ID/generation on every attempt and provider operation. Disconnect/reconnect must not turn an old attempt into failure or look it up through replacement credentials.
6. Enforce LNURL metadata-description hash, amount, callback URL policy, and HTTP validation; use or introduce a validated BOLT11 decoder with expiry/network/signature checks.
7. Prevent deep links from bypassing onboarding/risk consent and stop navigation from inferring origin by popping routes.
8. Rethrow `CancellationException` in every suspend adapter.
9. Pin the NWC dependency to an immutable release/artifact.

### P1: establish the rewrite foundation

1. Decide which provider app inherits each existing store identity and write the installed-user routing/handoff plan before creating targets. Ratify the product invariants and ADRs above.
2. Build core value types, typed errors/outcomes, and the payment-attempt transition model.
3. Introduce the app-specific database/vault namespaces, migration ledger, and golden fixtures for both `wallet.list` and `wallet.connection` eras.
4. Implement the `PaymentCoordinator` actor and reconciliation scheduler using a fake single backend.
5. Establish shared core/UI and provider-module boundaries, two DI compositions, dispatchers/scopes, navigation coordinators, redacted logging, and per-app CI gates.
6. Produce bootable NWC and Blink Android/iOS skeletons and prove provider code is absent from the opposite artifacts.
7. Port the regtest environment and make one fixed NWC payment vertical slice pass.

### P2: migrate product capabilities by vertical slice

1. NWC app connection/provisioning and fixed BOLT11 payment.
2. Blink app provisioning/payment with generated GraphQL and stable error mapping.
...
| Current single connection | `wallet.connection` |
| Legacy wallet aggregate | `wallet.list` |
| Legacy active wallet | `wallet.active` |
| Current Blink credential/default wallet | `blink.apikey`, `blink.defaultWallet` |
| Legacy per-wallet Blink values | `blink.apikey.<walletId>`, `blink.defaultWallet.<walletId>` |
...
The table describes data inside the current app sandbox. It does not imply that a new application/bundle ID can read it. The identity-successor decision in section 17.6 must be made separately for Android and iOS, including Keychain access-group and backup/restore behavior.

Migration must be idempotent and transactional where stores permit it. The existing single-connection migration is an input to the rewrite, not an adequate model for the split:

1. Read and validate legacy public records without deleting them.
2. Detect whether the source is the old multi-wallet aggregate or current single connection; never assume every installed user has already crossed the intermediate migration.
3. Generate a new `ConnectionId` and relationships in one database transaction for the one connection accepted by that successor app.
4. Copy credentials without logging or exposing plaintext to navigation/UI.
5. Verify the new profile-to-credential association and decryptability.
6. Record a migration version/checksum and the source app/schema.
7. Leave legacy data intact through the rollback window.
8. Delete old records only after product-approved success criteria and recovery support.
...
- Connection removal/replacement cannot redirect, erase, or falsely fail an in-flight attempt; lookup remains bound to the original `ConnectionId`.
...
- Both provider modules independently pass the same core payment contract, cancellation, and lookup tests.
- Legacy Android and iOS fixtures from both the multi-wallet and single-connection schemas migrate idempotently without secret loss.
- One explicitly chosen successor remains upgrade-compatible with each existing store identity/signing lineage; users of the other provider have a separately tested reconnect or secure-handoff path.
- Blink artifacts contain no NWC implementation/routes/SDK and NWC artifacts contain no Blink implementation/routes/Apollo operations; both app graphs and release tasks are verified.
...
- A real NWC regtest fixed payment, dynamic payment, duplicate, timeout/reconciliation, and disconnect/reconnect flow passes; the Blink app has an equivalent provider-environment or controlled contract/E2E gate.
...
- Do not create dozens of modules for a roughly 22K-line common codebase; create only the shared core/UI and provider boundaries needed to guarantee two clean application artifacts.
- Do not add a full functional-programming framework only to replace `Result`.
- Do not build a dynamic provider-plugin system, provider registry, or runtime provider switch for two provider-specific apps.
...
- An immutable connection generation per attempt.
...
Keep the cross-platform strategy, scanner-first experience, typed navigation concept, shared payment-backend contract, NWC session reuse, generated Blink API, native secret-storage intent, visual design system, and real regtest harness. Replace the mechanisms that make those decisions implicit: raw primitive/secret models, incomplete validation, fragmented state, mutable current-connection routing, session-only tracking, settings databases, service location, global event buses, and stale quality gates. Build two explicit product compositions around that shared core instead of carrying a runtime provider switch into the rewrite.
...
- `./gradlew :shared:testAndroidHostTest --tests "xyz.lilsus.papp.data.settings.WalletSettingsRepositoryImplTest" --tests "xyz.lilsus.papp.presentation.main.MainViewModelTest"` — **passed** on 2026-07-21 after the single-connection refactor.
- The 2026-07-21 delta review inspected commits `e465a45` (single connection) and `c49a006` (real-onboarding E2E setup), current provider/storage/payment code, migration tests, and app build composition.
...
| NWC bearer secret and provider union in ordinary domain model | [`WalletConnection.kt:7`](shared/src/commonMain/kotlin/xyz/lilsus/papp/domain/model/WalletConnection.kt#L7), [`WalletConnection.kt:20`](shared/src/commonMain/kotlin/xyz/lilsus/papp/domain/model/WalletConnection.kt#L20) |
| Single-connection migration retains one legacy wallet and deletes old selection data | [`WalletSettingsRepositoryImpl.kt:18`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/settings/WalletSettingsRepositoryImpl.kt#L18), [`WalletSettingsRepositoryImpl.kt:33`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/settings/WalletSettingsRepositoryImpl.kt#L33), [`WalletSettingsRepositoryImpl.kt:85`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/settings/WalletSettingsRepositoryImpl.kt#L85) |
| Blink credentials moved to global single-connection keys | [`BlinkCredentialStore.kt:8`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/blink/BlinkCredentialStore.kt#L8), [`BlinkCredentialStore.kt:51`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/blink/BlinkCredentialStore.kt#L51) |
...
| Error/result state remapped to Active while scanner uses original state | [`PayNavigation.kt:207`](shared/src/commonMain/kotlin/xyz/lilsus/papp/navigation/PayNavigation.kt#L207), [`PayNavigation.kt:715`](shared/src/commonMain/kotlin/xyz/lilsus/papp/navigation/PayNavigation.kt#L715), [`MainScreen.kt:121`](shared/src/commonMain/kotlin/xyz/lilsus/papp/presentation/main/MainScreen.kt#L121) |
| Pending state is cleared with ViewModel and unknown becomes failure | [`MainViewModel.kt:1685`](shared/src/commonMain/kotlin/xyz/lilsus/papp/presentation/main/MainViewModel.kt#L1685), [`PendingPaymentTracker.kt:322`](shared/src/commonMain/kotlin/xyz/lilsus/papp/presentation/main/PendingPaymentTracker.kt#L322), [`PendingPaymentTracker.kt:372`](shared/src/commonMain/kotlin/xyz/lilsus/papp/presentation/main/PendingPaymentTracker.kt#L372) |
| Mutable payment-service connection state, runtime provider branch, and constructor blocking | [`PaymentService.kt:29`](shared/src/commonMain/kotlin/xyz/lilsus/papp/domain/service/PaymentService.kt#L29), [`PaymentService.kt:42`](shared/src/commonMain/kotlin/xyz/lilsus/papp/domain/service/PaymentService.kt#L42) |
| Disconnect turns pending work into failure after explicit target removal | [`PendingPaymentTracker.kt:176`](shared/src/commonMain/kotlin/xyz/lilsus/papp/presentation/main/PendingPaymentTracker.kt#L176), [`PendingPaymentTracker.kt:189`](shared/src/commonMain/kotlin/xyz/lilsus/papp/presentation/main/PendingPaymentTracker.kt#L189) |
...
| JSON connection/contact writes and silent corruption fallback | [`WalletSettingsRepositoryImpl.kt:78`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/settings/WalletSettingsRepositoryImpl.kt#L78), [`WalletSettingsRepositoryImpl.kt:97`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/settings/WalletSettingsRepositoryImpl.kt#L97), [`ContactsRepositoryImpl.kt:212`](shared/src/commonMain/kotlin/xyz/lilsus/papp/data/settings/ContactsRepositoryImpl.kt#L212) |
...
| Whole-app DI graph, global scopes, both providers, hidden removal cycle | [`NwcModule.kt:115`](shared/src/commonMain/kotlin/xyz/lilsus/papp/di/NwcModule.kt#L115), [`NwcModule.kt:123`](shared/src/commonMain/kotlin/xyz/lilsus/papp/di/NwcModule.kt#L123), [`NwcModule.kt:284`](shared/src/commonMain/kotlin/xyz/lilsus/papp/di/NwcModule.kt#L284) |
