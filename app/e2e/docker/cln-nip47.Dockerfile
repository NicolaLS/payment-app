ARG CLN_IMAGE=elementsproject/lightningd:v25.12.1
FROM ${CLN_IMAGE} AS cln-nip47
...
COPY helper/server.py /usr/local/bin/papp-e2e-helper

EXPOSE 8080
ENTRYPOINT ["python3", "/usr/local/bin/papp-e2e-helper"]
