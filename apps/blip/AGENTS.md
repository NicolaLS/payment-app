...
- Shared application code lives in `shared/src/commonMain/kotlin/xyz/lilsus/blip`.
...
Do not commit secrets, wallet credentials, signing keys, local password files, or `e2e/.env.local`. Release signing can be configured through Gradle properties or `BLIP_RELEASE_*` environment variables; keep machine-specific paths out of shared changes unless intentional.
