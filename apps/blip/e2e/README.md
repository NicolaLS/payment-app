...
- Physical devices: set `TEST_NWC_RELAY_DEVICE_URL` to a `ws://...` URL the
  device can reach, and set `BLIP_E2E_BIND_HOST=0.0.0.0` before `e2e/bin/up`.

Useful overrides:

```bash
BLIP_E2E_RELAY_PORT=18081 BLIP_E2E_HELPER_PORT=18082 e2e/bin/up
BLIP_E2E_BIND_HOST=0.0.0.0 TEST_NWC_RELAY_DEVICE_URL=ws://192.168.1.20:8081 e2e/bin/up
```
...
The default Maestro suite targets the E2E app id `xyz.lilsus.blip.e2e`.
