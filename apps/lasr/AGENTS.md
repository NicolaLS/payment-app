...
> precedence over the historical structure described below. Lasr is the
> NWC-only app (`xyz.lilsus.lasr`); keep NWC-only stories and integrations under
> `apps/lasr`, and keep this app module focused on composition.
...
- Shared application code lives in `shared/src/commonMain/kotlin/xyz/lilsus/lasr`.
...
Do not commit secrets, wallet credentials, signing keys, local password files, or `e2e/.env.local`. Release signing can be configured through Gradle properties or `LASR_RELEASE_*` environment variables; keep machine-specific paths out of shared changes unless intentional.
