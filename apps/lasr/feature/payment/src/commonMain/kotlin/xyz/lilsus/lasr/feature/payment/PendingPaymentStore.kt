...
private fun StoredPendingRecord.toPendingRecord(): PendingRecord? {
    ...
    val normalizedStatus =
        when (restoredStatus) {
            PendingStatus.Sending -> PendingStatus.Resolving
            else -> restoredStatus
        }
    val restoredError =
        when (normalizedStatus) {
            PendingStatus.OutcomeUnknown ->
                NwcPaymentError.OutcomeUnknown("Payment status remains unknown after app restart")

            PendingStatus.Failed -> NwcPaymentError.Unexpected("Previous payment failed")
