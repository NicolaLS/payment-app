...
HOST = os.environ.get("PAPP_E2E_HELPER_HOST", "0.0.0.0")
PORT = int(os.environ.get("PAPP_E2E_HELPER_CONTAINER_PORT", "8080"))
PUBLIC_RELAY_URL = os.environ.get("PAPP_E2E_PUBLIC_RELAY_URL", "ws://127.0.0.1:8081")
NODES = {
    "payer": "/cln/payer",
...
class Handler(BaseHTTPRequestHandler):
    ...
    def handle_get_nwc_uri(self, query):
        ...
        label = first(query.get("label"), f"papp-e2e-{node}")
    ...
    def handle_create_invoice(self):
        ...
        label = body.get("label") or f"papp-e2e-{int(time.time() * 1000)}"
        description = body.get("description") or "Papp E2E invoice"
...
def main():
    ...
    print(f"Papp E2E helper listening on {HOST}:{PORT}", flush=True)
