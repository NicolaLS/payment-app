...
The experiment on `main` connects Claro El Salvador airtime and packages through
...
On `main`, mobile development builds accept
`-Prayl.hub.baseUrl=https://your-configured-host` or the `RAYL_HUB_BASE_URL`
environment variable. The Gradle property takes precedence. With no URL, neither
native platform creates a remote session or order store; local Hub features
continue to work. The URL is build configuration, not a user account setting.

The `release/rayl/1.0` and `release/blip/1.0` branches contain the local Hub and
metric foundation, before the Claro purchase experiment. Their source
configuration fixes the backend URL to empty and ignores both URL overrides.
Backend services require a later mobile release; the experimental contract is
not a public 1.0 compatibility promise.
