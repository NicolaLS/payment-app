...
import xyz.lilsus.raylsuite.core.payment.EncodedLightningInvoice
...
class PaymentCoordinator(
            ...
            is LightningInputParser.ParseResult.Success ->
                when (val target = result.target) {
                    is LightningInputParser.Target.Bolt11 -> {
                        pendingTracker.findWaitingByPaymentRequest(target.invoice.write())
                            ?.let { existing ->
    ...
    private fun startPayment(
        ...
        val job =
            scope.launch {
                try {
                    val result =
                        paymentProvider.payInvoice(
                            PayInvoiceRequest(
                                invoice = EncodedLightningInvoice(invoice.write()),
    ...
    private fun handlePaymentSuccess(
        ...
        showPaymentSuccess(
            CompletedPayment(
                ...
                feeMsats = feeMsats,
                showEstimatedFeeHint = showEstimatedFeeHint && !result.wasAlreadyPaid,
                wasAlreadyPaid = result.wasAlreadyPaid,
                preimage = result.preimageHex
            )
        )
    ...
    private fun handlePaymentFailure(
        ...
        val providerError = (error as? PaymentUiError.Provider)?.error
        if (providerError is PaymentError.PaymentUnconfirmed) {
            handlePaymentUnconfirmed(
                pendingId = pendingId,
                invoice = invoice,
                amountOverrideMsats = amountOverrideMsats,
                paymentHash = providerError.paymentHash
            )
            return
        }
        if (providerError == PaymentError.Timeout) {
            handlePaymentUnconfirmed(
                pendingId = pendingId,
                invoice = invoice,
                amountOverrideMsats = amountOverrideMsats,
    ...
    private fun handlePaymentUnconfirmed(
        ...
        pendingTracker.startVerification(
            id = pendingId,
            summary = invoice,
            amountOverrideMsats = amountOverrideMsats,
            paymentHash = paymentHash ?: PaymentHash(invoice.paymentHash.toHex())
        )
    }
    ...
    private fun refreshNewSessionTransactionCount(items: List<SessionTransactionItem>) {
        val unseenIds = items.map { it.id }.filterNot(knownSessionTransactionIds::contains)
...
private fun Throwable.toPaymentUiError(): PaymentUiError = when (this) {
    is PaymentException -> PaymentUiError.Provider(error)
...
private fun LnurlError.toPaymentUiError(): PaymentUiError = when (this) {
    LnurlError.NetworkUnavailable ->
        PaymentUiError.Provider(PaymentError.NetworkUnavailable)
