...
fun rememberPaymentFlowState(coordinator: PaymentCoordinator): PaymentFlowState {
    ...
    return PaymentFlowState(
        payment = uiState.toPaymentScreenState(::flintPaymentErrorMessageFor),
        sessionItems =
...
/** Localized snackbar text for Flint's payment errors and unsupported-input toasts. */
@Composable
fun rememberPaymentMessages(coordinator: PaymentCoordinator): Flow<String> {
    ...
    return remember(coordinator, context) {
        coordinator.events.map { event ->
            when (event) {
                is PaymentEvent.ShowError -> getFlintPaymentErrorMessageFor(event.error, context)
...
private fun SessionTransactionItem.toPaymentTransactionDetail() = PaymentTransactionDetail(
    id = id,
    state = toDetailUiState().toPaymentScreenState(::flintPaymentErrorMessageFor),
